...
export class LoginPage {
	...
	afterDriverGet(driver){
	  ...
	  AddTokenToDB(token,that){
		...
		this.oneService.editDriver(data).subscribe((res)=>{
		  console.log(res);
	...
	doLogin() {
		...
		loading.present();
		this.oneService.login(this.loginForm.value).subscribe(	response => {
				loading.dismiss();
				if (!response.error) {
					localStorage.setItem("driver", JSON.stringify(response.data));
					this.driver = response.data;
					 this.afterDriverGet(response.data);
