    ...
    this.threeService.getOrders(allowids).subscribe((data) => {
      ...
      if (!data.error) {
        if (data.message.length > 0) {
          ...
          let pendingorders = data.message.filter((index) => {
          ...
          let completeOrders = data.message.filter((index) => {
...
export class OrderDetailPage {
  ...
  CheckMinimumDelivery(deliverytime){

    let today = moment();
  
    let newdeliver = moment(deliverytime);
    let current = moment();
    newdeliver.subtract({ hours: 1, minutes: 1});
 
    let response = today.isAfter(newdeliver);
...
changePackageStatus(status,orderid,pkg){
  ...
  Notification(order,status){
     let token = [];
     this.oneService.Getowner(order.restaurantData.ownerId).subscribe((res:any)=>{
       ...
       let userid = order.customerData._id;
       let ownerid = order.restaurantData.ownerId;
       this.oneService.Notification(data).subscribe((res)=>{
        let data2 = {_id:userid,fcmToken:res.message.usertoken  };
        let data3 = {_id:ownerid,fcmToken:res.message.ownertoken};
         this.oneService.updateCustomer(data2).subscribe(()=>{
               this.oneService.editOwner(data3).subscribe(()=>{
          ...
          },
          {
            ...
            handler: data => {
              ...
              if(status == 'driveraccepted'){
                ...
                this.driver.orderIds.push(this.orderDetail._id);
                this.oneService.editDriver(this.driver).subscribe(data=>{
                  if(!data.error){
                    console.log("udpate");
                  }
                })
                this.Notification(this.orderDetail,"accepted");
                this.oneService.getDriver(this.driver._id).subscribe(data=>{
                  console.log(data,"updated driver");
                })
                var obj1 = {customeremail: this.orderDetail.customerData.email, order: this.orderDetail};
                console.log("I Reached 1");
                this.oneService.EmailDriverAccepted(obj1).subscribe(()=>{
                  console.log("Email Done")
                })
                this.orderDetail.driverDetail = this.driver;
                
              }else if(status == "ontheway"){
                this.Notification(this.orderDetail,"ontheway");
                var obj1 = {customeremail: this.orderDetail.customerData.email, order: this.orderDetail};
                this.oneService.EmailOntheway(obj1).subscribe(()=>{
                  console.log("Email Done")
                })
              }else if(status == "delivered"){
                this.Notification(this.orderDetail,"delivered");
                var obj1 = {customeremail: this.orderDetail.customerData.email, order: this.orderDetail};
                this.oneService.EmailDelivery(obj1).subscribe(()=>{
                  console.log("Email Done")
                })
              }
