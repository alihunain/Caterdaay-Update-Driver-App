...
export class MyApp {
  ...
  constructor(
    ...
    this.pages = [
      ...
      { title: 'Caters', component: RestaurantsPage },
 ...
 async registerRequestPermission() {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: data => {
            ...
            this.updateTokenDB().then((token)=>{
             
              this.oneService.editDriver(token).subscribe(()=>{
  ...
  updateTokenDB(){
    ...
    FCMPlugin.getToken(function(token){
      let t = token;
      let id = JSON.parse(localStorage.getItem("driver"))._id;
      that.oneService.getDriver(id).subscribe((res)=>{
